...
window.PROJECT_MAP = {
  ...
  nodes: [
    ...
    },
    {
      ...
      desc: "One rt process, any number of windows (App::windows keyed by WindowId; Ctrl+Shift+I opens one, each closes independently, rt exits with the last). A pane or tab tears out with a key (Ctrl+Shift+D / Ctrl+Shift+J) or by dragging it to bare desktop. In-window drag (pane by titlebar, tab by label) gives live cues — half-pane split fill, whole-pane swap, tab-insert caret, window-edge root-split band, a ghost chip, source dim — on both backends. Cross-window drag-with-cues (the single fluid button-held gesture) is X11-only (gated on winit's inner_position, which Wayland never answers); drag tear-out works on Wayland too (surface-local out-of-bounds release, compositor-placed — verified on cosmic-comp). Carry mode now covers cross-window drops everywhere: Ctrl+Shift+M/N pick up the focused pane/tab, or Ctrl-held release a drag outside the window, then hovering ANY rt window (Wayland or X11) shows the full drop-cue set and a left-click commits with the same semantics as the live drag; Escape or a right/middle-click cancels. While a button is held mid-drag, or throughout a carry, the pointer wears a held-pane card cursor (accent outline, translucent body, opaque titlebar band, pane aspect ratio) built as a pure-RGBA custom cursor; it reverts to the foreign app's or desktop's own cursor once the pointer leaves every rt window (cursor ownership stays with that surface on both Wayland and X11), and falls back to plain CursorIcon::Grabbing if the compositor refuses custom cursor images. A cross-window move or tear-out cuts any patch-bay wire that would end up spanning two windows; a wire wholly inside the moved payload travels with it.",
      ...
      parts: [
        ...
        { label: "Carry mode (pick up, aim, click to drop)", status: "done", desc: "Ctrl+Shift+M/N pickup (also \"Pick Up Pane\"/\"Pick Up Tab\" menu rows) or a Ctrl-held drag-release outside the window enters carry; every rt window resolves its own hover cues, a left-click commits cross-window on Wayland and X11 alike, Escape/right/middle-click cancels. Works with titlebars off." },
