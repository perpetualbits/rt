  ...
  nodes: [
    /* ---- Frontend & Rendering ---- */
    {
      ...
      parts: [
        ...
        { label: "Desktop integration (Linux .desktop, macOS .app)", status: "done", desc: "extra/linux installs a freedesktop .desktop entry and icon theme PNGs; extra/macos builds rt.app — Info.plist (identifier io.github.perpetualbits.rt, the same identity as the desktop file; NSHighResolutionCapable true so a Retina backing store is not faked; LSMinimumSystemVersion 11.0, read off the binary's LC_BUILD_VERSION rather than guessed; the NS…UsageDescription set a terminal needs because TCC attributes a pane's privacy request to the bundle), an .icns generated from extra/logo/rt.svg, and an ad-hoc code signature. A Finder launch has no shell environment: measured on the Mac, PATH/SHELL/HOME/USER all come back correct because the pane's shell goes through /usr/bin/login and path_helper, but the working directory is / — app_bundle.rs moves a bundled rt to $HOME before the first pane, and only then. Bundle structure, plutil, codesign, mdls and lsregister verified on macOS 26.6.2; the double-click itself is still unwitnessed. extra/macos/dmg.sh wraps rt.app into a distributable rt-<version>-macos-arm64.dmg (UDZO, Applications-symlink drag install; unsigned/arm64-only, both asserted at build time — refuses a non-arm64 host and lipo-checks the bundled binary); extra/RELEASE.md is the end-to-end cut-a-release recipe. Shipped v0.3.24, then v0.3.25: built, mounted and inspected on kiku, then published as a real GitHub release asset each time. The separate deb/rpm/arch packaging job (cargo-zigbuild, glibc floor 2.28 x86_64/aarch64 + 2.35 riscv64) had been failing on both tags — rt's own extern \"C\" gettid() declaration hard-required a glibc-2.30+ symbol; switched to the raw SYS_gettid syscall (no version floor) and shipped the fix in v0.3.26." }
      ],
    ...
    },
    {
      ...
      desc: "The macOS rendering backend. Same Backend trait as the GL and XRender paths, but on wgpu over Metal, with its own glyph atlas mirroring render.rs (one WGSL shader, one R8Unorm atlas, texel (0,0) forced opaque so solid fills share the glyph pipeline). Compiled only on macOS: the Linux crate graph is byte-identical, enforced by ci/check-target-deps.sh. Exposes no damage capabilities (no partial present, no buffer age, no scroll blit), so it always redraws the full frame — measured on an M5 and upheld: a full 177x61 redraw at 3024x1964 costs 396us, 2.4% of a 60Hz frame, while the only sound partial path (a persistent surface-sized texture blitted every frame) has a 118us floor of its own and needs 23.8 MB per window. The scissored draw path is nonetheless correct now: a scissor cannot clip a load-op clear, and PxRect's signed origin cannot be cast to wgpu's u32 scissor, so both are decided in wgpu_frame.rs and gated by real offscreen Metal pixel tests.",
      files: ["crates/rt/src/wgpu_backend.rs", "crates/rt/src/wgpu_text.rs", "crates/rt/src/wgpu_frame.rs", "crates/rt/src/wgpu_offscreen.rs", "crates/rt/src/vibrancy.rs", "crates/rt/src/vibrancy_policy.rs", "crates/rt/src/scale_policy.rs", "crates/rt/src/chrome_scale.rs"],
      specs: ["docs/superpowers/specs/2026-09-03-macos-port-design.md"],
      parts: [
        { label: "Glyph atlas", status: "done", desc: "fontdue rasterisation + preference-chain face resolution, mirroring render.rs. The cache is keyed (char, bold, italic) in an FxHashMap: SipHash on that key was 24% of a frame on a U74 (docs/software-gl-lessons.md) and the swap is worth ~8% of the vertex build on an M5 too, measured. The GL renderer's other half of that fix — a direct-indexed ASCII table that hashes nothing — is deliberately NOT here: on an M5 it is a 25% regression, reproduced across three index/payload layouts. Font reload invalidates the glyph and mask caches in place (the pipelines and atlas are font-independent), gated by offscreen Metal tests." },
        { label: "HiDPI scaling", status: "done", desc: "Cell metrics follow the backing scale factor, so Retina text is not half-size — at startup, and now also when the factor CHANGES under a running window (dragged between a Retina panel and a 1x monitor, or the display's own factor changing). A scale change arms the same RESIZE_SETTLE the deferred-resize path uses rather than re-measuring inline, so a slow drag across a display boundary pays one font reload and one reflow at the resting scale instead of one per event; painting is suspended in between so no frame shows old cell metrics on the new surface. The decision is pure data in scale_policy.rs, unit-tested on Linux like vibrancy_policy.rs; not macOS-only, since a HiDPI Wayland/X11 output takes the same path. All hit-testing is physical-pixel throughout — audited consumer by consumer (mouse, panes, titlebars, colour picker, jacks, menus, prefs, scrollbars, drag-and-drop) with no logical/physical mismatch found. The CHROME follows the factor too: every flat pixel constant (window margin, pane padding, split gutters and their grab band, tab strips, patch-bay jack discs AND the radius that hits them, scrollbar and its grab slop, drop cues, drag threshold, and every overlay panel's padding) is a LOGICAL value multiplied by the backing factor, registered in one frozen table in chrome_scale.rs. Draw and hit always take the same factor; at 1.0 the multiply is the exact identity, so Linux is bit-for-bit unchanged." },
        { label: "Frosted glass", status: "done", desc: "Two mechanisms, one state machine. The DEFAULT is now the Terminal.app one: a plain, UNTINTED window-server backdrop blur at a radius rt chooses (CGSSetWindowBackgroundBlurRadius, private SPI, failing soft), so the only colour on screen is the configured background at the configured opacity. rt makes that call itself rather than via winit's set_blur, which hardcodes radius 80 — the radius is macos_blur_radius (1-100, default 24) with its own live Preferences row. Every NSVisualEffectMaterial stays reachable for anyone who wants vibrancy and light/dark adaptation, installed as a sibling below the content view; each carries its own tint, which is why one is no longer the default. Both are gated on the same wants_background_blur() predicate as the Wayland/X11 blur paths and re-applied live on every opacity/settings change; switching between them removes the effect view and clears the radius in the same pass. The whole install/remove/retarget/radius decision is pure data in vibrancy_policy.rs (glass_plan) so Linux CI tests it." }
      ],
      deps: []
    },
    {
      ...
      parts: [
        ...
        { label: "Re-measure the ASCII glyph fast path", status: "planned", desc: "wgpu_text.rs uses an FxHashMap only. The direct-indexed ASCII table that made the GL path 24% faster on a U74 is a 25% REGRESSION on an M5 and was left out; the U74 is the machine it was built for, so measure it there before adding it back — separate builds alternated, not one process (docs/software-gl-lessons.md)." }
      ],
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "Hardware GL on a slow CPU", status: "done", desc: "v0.3.21. With the PowerVR driver live on the Mars, rt still held ~0.9 cores of its own main thread: hardware GL always took the full-frame path, draw_panes walked all 7000 cells even under a scissor, and SipHash on the glyph key was a quarter of every frame. Now buffer age picks partial frames on every GL backend, geometry is built only near the applied clip (one-cell margin), ASCII glyphs are a table lookup (rustc-hash for the rest), and instrument animation on hardware GL runs at ~30fps; damage history keeps 4 frames (NVIDIA reports buffer age 3, so at 2 every frame was silently full). dop561/NVIDIA, same harness: post-flood easing 0.63 -> 0.01 cores, median frame 24 ms full -> 2.3 ms partial. v0.3.22: partial-frame bands now reach the rim instruments' radius on both sides of each pane edge and cover the window latency frame (the packets orbit ON the perimeter; 6px inside-only bands halved every jack and froze the packets)." }
      ],
      deps: []
    },
    {
    ...
    {
      id: "text-drop", label: "Dropped-in text", layer: "frontend", status: "done",
      tags: ["macOS", "X11", "Wayland"],
      desc: "Text dragged in from ANOTHER application (select in Chrome/Firefox, drag onto a pane, drop) is inserted like a paste \u2014 the feature terminator and gnome-terminal have and Alacritty does not. winit surfaces FILE drops only (WindowEvent::DragEntered carries Vec<PathBuf>; its macOS delegate registers NSFilenamesPboardType alone and refuses everything else), so each platform needs a receiver below winit. macOS is done: an NSDraggingDestination NSView registered for NSPasteboardTypeString, added as a click-through \"glass\" subview of winit's content view (hitTest: returns nil, so mouse/IME/cursor still land on winit's view) writing into an Rc<RefCell<DropInbox>> the run-loop reads in about_to_wait. The pane under the POINTER takes the drop (not the focus) and then becomes the focus; delivery is Session::paste_to_pane, the same per-pane bracketed-paste wrap + end-marker strip feed_paste uses, so it reaches exactly one pane and cannot break out of its own bracket. Both decisions \u2014 which pane, and what bytes \u2014 live in the un-cfg'd textdrop module so Linux CI tests them. X11 is done too, by a different mechanism: winit-x11 speaks XDND but hardcodes text/uri-list, and XDND client messages reach only the window's creating client, so rt names a 1x1 window of its own as the toplevel's XdndProxy and receives the whole exchange there. Wayland is done too, and it is the one that cost something: a client receives drags on a wl_data_device, and rt's clipboard (smithay-clipboard) already owned the only one it may safely have. Mutter answers a client's SECOND get_data_device by unlinking the first into a self-loop -- alive, in no list, never sent `selection` again, so paste dies silently on GNOME with no error and no log. Smithay/cosmic-comp, KWin and wlroots all fan out to every device and would have been fine; three of four is not good enough for a clipboard. So smithay-clipboard's code came in-tree verbatim (MIT) and its EMPTY drag callbacks were filled in: one device, doing clipboard, PRIMARY and drag-and-drop. rt creates no wl_data_device it did not create before the feature existed.",
      files: ["crates/rt/src/textdrop.rs", "crates/rt/src/text_drop_mac.rs", "crates/rt/src/text_drop_x11.rs", "crates/rt/src/wl_clipboard/state.rs", "crates/rt/src/wl_clipboard/dnd.rs", "crates/rt/src/main.rs", "crates/rt/src/chrome/dragdrop.rs", "crates/rt-session/src/lib.rs"],
      links: [
        { label: "Wayland text drop (and why it took the clipboard with it)", href: "docs/wayland-text-drop.md" }
      ],
      parts: [
        { label: "Drop policy (pure, tested)", status: "done", desc: "textdrop::resolve picks the pane under the cursor and refuses tab strips, gutters and open overlays; textdrop::payload normalises CRLF, never delivers a trailing newline, and flattens line breaks when the target has no bracketed paste. The Wayland slice added pick_drop_mime (the accept/reject decision, answering with the SOURCE's own spelling because wl_data_offer.accept/receive match it verbatim), surface_to_physical (surface-local logical coordinates -> physical pixels) and may_finish_drop (the three preconditions on wl_data_offer.finish -- getting it wrong is the invalid_finish protocol error, which tears down rt's whole wl_display). 29 unit tests, run on Linux." },
        ...
        { label: "Wayland receiver", status: "done", desc: "On the wl_data_device rt's clipboard already owns -- never a second one (Mutter's get_data_device unlinks the client's existing device, silently killing paste on GNOME; verified in meta-wayland-data-device.c, and against Smithay/KWin/wlroots which all fan out safely). smithay-clipboard 0.7.3 is now in-tree verbatim (MIT, five import lines changed, landed as a behaviour-free commit so the clipboard delta is auditable) with its empty DataDeviceHandler drag stubs filled in; the dependency graph lost one crate and gained none. Copy-only actions, a 3s transfer deadline, and a guarded wl_data_offer.finish. The worker thread wakes the loop through winit's EventLoopProxy, so the cue tracks the pointer instead of waiting on the 100ms idle poll the X11 receiver pays. See docs/wayland-text-drop.md." },
    ...
    /* ---- Chrome & Interaction ---- */
    {
      ...
      tags: ["v0.3.12"],
      desc: "All the ways you grab text: drag-select with copy-on-release, wrap-aware double-click word and triple-click logical-line select, and the new anchored mode — Shift+click drops a start anchor, you navigate freely (accelerating arrows, scrollbar, Page/Home/End) with the anchor pinned to its content, and a second Shift+click or Enter commits and copies. Ctrl gives a rectangular block. The Wayland CLIPBOARD and PRIMARY backend moved in-tree with the text-drop work (crates/rt/src/wl_clipboard, smithay-clipboard 0.7.3 verbatim under MIT): the same wl_data_device now serves copy, middle-click paste and dragged-in text, because a client cannot safely own two -- see Dropped-in text. The clipboard code paths themselves are byte-identical to what shipped.",
      files: ["crates/rt/src/select.rs", "crates/rt/src/clipboard.rs", "crates/rt/src/wl_clipboard/mod.rs"],
    ...
    },
    {
      ...
      parts: [
        ...
        { label: "Predictable pane colours", status: "done", desc: "The same defect as the palette's, one level in: the per-pane titlebar strip, its hairline and the newspaper-column rule derived their tint from the CONFIGURED background. They are now measured against theme::surround — what the composite actually puts on screen — in two regimes with a crossfade between them, so nothing about a nearly-opaque window changes and a sheer one stops washing out. While at most BAR_UNKNOWN_MAX (10%) of the pane is unknown backdrop, the band is the pane body lifted toward the foreground at opacity*(to-from)/(1-from), bit-for-bit what rt drew before — 10% unknown is the point up to which surround is still within FLOOR_EDGE of the configured background for every scheme rt ships, i.e. the composite is not even findable as a different surface, so a lift measured in the configured span still lands where the user asked. Below that the band becomes mix(bg, fg, to) — the titlebar colour an OPAQUE window shows — pulled toward the pane's own background by however much that colour falls short, against surround, of the separation it has on an opaque window. The reported defect (\"the background color in the title bar in OSX is still too bright\") was the first rule applied where the second belongs: on gold-on-black at background_opacity 0.65 the strip came out [71,65,46] over a [43,43,52] body — a gold wash riding on the backdrop's own lift — and now comes out [15,15,18], a tenth of the luminance at slightly BETTER separation (1.368:1 vs 1.365:1). At 0.90, the setting Linux runs at, the fill is identical to the last bit. The fill alpha is capped at the pane's own opacity — the strip may be denser glass, never more than twice the body's density — which is what keeps a fully see-through window free of a titlebar slab. A backend that cannot composite (XRender, PictOp::SRC) keeps the opaque pre-mix rather than being handed an alpha it would draw as a full-strength slab." },
    ...
    },
    {
      ...
      parts: [
        { label: "Presets are non-destructive", status: "done", desc: "Trying a built-in preset used to overwrite foreground/background/palette with nothing holding what was there, and commit_settings persists — so one keypress took the user's colours out of config.toml too (\"I had custom colors set, but when I tried one of the presets, my custom scheme is gone\"). Settings::custom_scheme now remembers the last colours that matched no built-in, and rt_config::scheme_candidates puts them in the cycle as a real \"custom\" entry beside the presets — the same promise term_candidates keeps, that the value in force is always one of the choices. Stepping off custom and back restores fg, bg and the whole palette. The invariant is maintained in one place, Settings::remember_custom, called from every colour write: the preset row, the colour picker's slot write (so the picker cannot be a second route to the same loss — a swatch edited while sitting on a preset becomes the custom scheme there and then), and normalize() on load, which is how a config.toml written by an older rt or by hand arrives with its colours already in the cycle. No custom entry appears for a user who has never had one. The field is last in Settings because Some serialises as a TOML table; an older rt ignores the unknown key and keeps the rest of the file." }
      ],
      deps: ["chrome-prefs", "chrome-theme", "rt-config"]
    },
    {
      id: "chrome-menu", label: "Menu & manual", layer: "chrome", status: "done",
      tags: ["native chrome"],
      desc: "The right-click context menu that drives rt actions, and the built-in F1 manual — both drawn natively. The menu shares its action table with the keymap so a binding and its menu item can never drift apart. On macOS that same table drives two NATIVE renderers instead — the system menu bar, where a Mac user looks for a program's menu, and a real NSMenu on right-click — and rt draws no menu of its own there at all.",
      ...
      specs: [],
      parts: [
        { label: "macOS menu bar", status: "active", desc: "A real NSMenu in the system menu bar — Shell / Edit / View / Window / Help, plus Settings… in the application menu winit already installs (⌘Q and ⌘H untouched). EVERY one of rt's 49 Actions is up there: the guard test now asserts against Action::ALL (generated by rt-config's actions! macro from the list that declares the enum) rather than against the context menu's subset, so a new variant fails the build until it is filed in a category or excluded by name with a reason. The fourteen that had no menu at all — the patch bay (Shell), the opacity pair (View), pane focus and divider moves (Window) — are filed. Rows, labels and enabled-state come verbatim from menu::rows(), so a click dispatches the same Action a keybinding does via App::apply_action. Chords are real AppKit key equivalents taken from the keymap. Enable/disable is live via -validateMenuItem: against a snapshot the run loop refreshes each turn, which also greys the bar (and kills its key equivalents) while a modal overlay owns the keyboard. The click reaches the loop through winit 0.31's EventLoopProxy::wake_up + proxy_wake_up. The AppKit calls are macOS-only; the STRUCTURE is plain data in menubar_model.rs, unit-tested on Linux like vibrancy_policy.rs. Awaiting on-screen verification." },
        { label: "macOS right-click popup", status: "active", desc: "On macOS rt draws no menu of its own: a right-click pops a real NSMenu, with the system's appearance, shadow, scrolling and keyboard handling. chrome::menu is never reached there because main.rs never sets Active::menu on that target (Linux keeps the self-drawn menu untouched). The rows are menu::rows() verbatim — same order, labels, separators, enable rules and version footer — INCLUDING the three pointer-dependent ones the menu bar cannot have (Open Link, Copy Address, Move Pane to …), each still carrying its MenuPick. The show is deferred one turn with performSelector:withObject:afterDelay:inModes: (common modes, matching winit's own run-loop observer) because popUpMenuPositioningItem: is modal and must not start a nested loop with winit's handler on the stack; the pick comes back via the same EventLoopProxy::wake_up queue the bar uses, and an action row goes through run_menu_bar_action → apply_action. It answers the same suspended gate as the bar, from one shared predicate. popup_model() is plain data in menubar_model.rs and is tested on Linux; the AppKit calls rest on a clean macOS compile." },
