...
- **Text dropped in from other apps** *(X11 and macOS; native Wayland still to come)* — select text in a browser or an editor, drag it onto a pane, let go: it is inserted there like a paste, into the pane **under the pointer** (which then takes focus, so your Return goes where the text went). The receiving pane lights up while you hover; the tab strip and open dialogs refuse the drop rather than typing into a pane you cannot see. Multi-line text is bracketed-pasted so a shell stages it without running it, and with bracketed paste off the line breaks become spaces — a dropped paragraph can never turn into a series of commands. winit surfaces file drops only (and on X11 only `text/uri-list`), so this is rt's own receiver below it: an `NSDraggingDestination` on macOS, an XDND `XdndProxy` window on X11.
