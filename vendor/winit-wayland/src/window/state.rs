...
impl WindowState {
    ...
    /// Drop a move that was armed by a click on the frame but never acted on.
    ///
    /// Upstream arms `has_pending_move` on the press and fires it from the next
    /// `frame_point_moved`. A touch that taps the title bar and lifts produces
    /// no such motion, so without this the serial would sit there and turn some
    /// later, unrelated hover into a window move nobody asked for.
    pub fn frame_cancel_pending_move(&mut self) {
        self.has_pending_move = None;
    }
